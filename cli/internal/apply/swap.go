...
// AsideSuffix is record.AsideSuffix, re-exported so no call site in this
// package retypes the literal. There is exactly one definition of it in the
// CLI, and that is what makes an entry's removable set computable rather
// than discoverable: record.Entry.RemovablePaths() is {Dest, Dest+AsideSuffix}.
// A second literal here is how the swap and the prune drift apart and leave
// an aside nothing ever removes.
...
//
// There is deliberately no recursive-copy fallback. A copy is not atomic, so
// a fallback would silently invert the single requirement this code exists
// for: a destination that can be observed half-written. Failing the entry
// and naming both paths is the correct outcome — the fix is to put the
// staging directory on the destination's filesystem, which
// layout.StagingRoot already does, so reaching this error means a mount
// point or a symlink sits between them.
...
type SwapResult struct {
	...
	// Reclaimed says step 1 found a leftover aside with no destination
	// beside it and moved it back: an earlier run of this entry died in the
	// one window where the destination is absent. Worth reporting — it
	// means a previous sync was interrupted mid-install.
	Reclaimed bool

	// DiscardedAside says step 1 found a leftover aside with a destination
	// already in place and removed it: an earlier run died after the new
	// version was installed but before its aside was cleaned up.
	DiscardedAside bool

	// SyncDirErr is step 4's failure, if any. Non-fatal: the new version is
	// already in place and readable, and the record is written after the
	// swap, so a lost rename loses the record write with it rather than
	// leaving the record claiming an entry that is not there. The fsync
	// narrows that window; it does not create the safety.
	SyncDirErr error

	// RemoveAsideErr is step 5's failure, if any. Non-fatal: the new version
	// is already in place, so failing the entry here would report a broken
	// install for a working one. See LeftoverAside.
...
// by a five-step sequence. staging must be inside dest's parent directory —
// layout.StagingRoot builds such a path — and is consumed by a successful
// swap.
//
...
//     ErrCrossDevice.
//   - It does not follow a symlink at dest, and it does not refuse one
//     either. The link itself is renamed aside and then removed, so nothing
//     is written through it, since following one is exactly how amctl would
//     write outside the home without ever constructing a path outside it.
//     The guard is the caller's: a symlink at a destination is by
//     definition not something amctl wrote (extraction refuses symlink
//     members), so apply must refuse the entry unless --force is given, and
//     --force must name the link it destroys. Swap is unconditional by
//     design.
...
//     anything is opened.
//   - It does not write the installation record. The record is written
//     after the swap, never before: a record claiming an entry that is not
//     on disk causes a spurious removal attempt, while an entry on disk
//     with no record is merely re-installed next run.
func Swap(staging, dest string) (SwapResult, error) {
	...
	// STEP 1 — reclaim or discard a leftover aside from an earlier
	// interrupted swap. Fatal: an aside in the way makes step 2 fail
	// forever, since Go refuses any existing directory as a rename
	// destination, empty or not.
	//
	// It reclaims rather than discards when dest is absent. A crash between
	// steps 2 and 3 leaves the aside holding a complete old version —
	// rename is atomic, so it is never partial — and discarding it would
	// destroy the only copy of the version the record claims, leaving this
	// run's step 3 nothing to roll back to.
	...
	// STEP 3 — move the staged tree into place. dest is absent here, which is
	// the only shape os.Rename accepts for a directory on any platform, and
	// is why step 2 is not a nicety but the whole mechanism.
	//
	// Fatal with rollback: after step 2 the machine is changed, so a failure
	// here needs the old version put back before the error is returned. A
	// crash cannot do this, which is what step 1 is for.
	...
	// STEP 4 — fsync dest's parent so step 3's directory entry survives a
	// power loss. Non-fatal. This makes the directory entry durable, not
	// the file content underneath it: on a delayed-allocation filesystem a
	// power loss here can leave dest present and full of zero-length files,
	// which no care in this function can prevent. Content durability is
	// Stage's — it fsyncs every extracted file before handing the tree over.
