...
//
//   - It touches NOTHING installed (FR-008). It removes one credential and
//     returns. It does not read the installation record, does not walk a target
...
//     already on disk. Somebody logging out to switch hubs would otherwise find
//     their skills deleted. `amctl sync` is what removes packages.
//   - It makes NO network request, and constructs no hub client. That is not an
//     optimisation. hub.New refuses an http:// URL without
//     --allow-plaintext-hub (FR-041), so building one here would mean a
//     credential stored for a plaintext hub could not be deleted without
//     re-typing the flag that permitted it — leaving the token on disk, which
//     is the opposite of what the user asked for. It also means logout works
//     with the hub unreachable, which is when people reach for it.
//   - It does not remove the ~/.agent-manager/credentials directory, or the
//     per-hub state directory. Another hub's credential lives in the first
//     (FR-006) and the installation record lives in the second.
...
//     provisioning script that may or may not have logged in must not fail. The
//     `removed` field is how a script tells the two apart without parsing prose.
...
// validates the home directory before the hub URL, and the credential key is
// Hub.URL — the CANONICAL form. That matters more than it looks. login stores
// under the same canonical string, so `logout --hub HUB.Example.com.` removes
// the credential `login --hub https://hub.example.com/` wrote; a second opinion
// on canonicalisation anywhere in this path would produce a logout that
...
func runLogout(opts *Options, deps logoutDeps) error {
	...
	return Prepare(opts.Hub, func(home Home, target Hub) error {
		...
		if token, ok := lookupEnv(deps.lookupEnv)(credentials.TokenEnvVar); ok && token != "" {
			// FR-005: the environment token outranks the store, so removing the
			// stored credential does not log this shell out. A logout that let
			// someone believe otherwise is worse than no logout.
