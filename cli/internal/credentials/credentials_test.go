...
// sentinelToken is the value every test that stores anything uses,
// deliberately loud so a leak of it into any output is unmistakable rather
...
// keychain, the session's Secret Service or KWallet, a GPG password store.
// The file backend is not one of them, since Options.StateRoot puts it
// inside the test's own t.TempDir.
//
// They are off by default because on a macOS CI runner this suite once hung
// for six and a half minutes blocked in SecItemCopyMatching — the login
// keychain asking a human for permission nobody was there to give. The same
// code on a developer's Mac pops a dialog during `go test ./...` and, if
// answered, leaves amctl's test items in the real keychain. Store adds no
// per-backend logic, so what these subtests cover beyond the file backend is
// keyring's own behaviour and the OS's. Set AMCTL_TEST_SYSTEM_KEYRING=1 to
// run them deliberately, on a machine where a prompt has somewhere to
// appear.
...
// openableBackends is the policy order intersected with what actually opens
// on this machine and what this suite may write to. The gate is on the
// backend's kind, not on whether it opened: keyring.Open on the macOS
// keychain succeeds without touching the keychain, and the first Get blocks
// on an authorisation prompt.
...
func TestDeletingACredentialThatIsNotThereIsSuccess(t *testing.T) {
	for _, backend := range openableBackends(t) {
		t.Run(string(backend), func(t *testing.T) {
			...
			// `amctl logout` on a machine that never logged in, and `amctl
			// logout` run twice, must both succeed. The file backend's
...
func TestTwoHubsCoexistWithoutTouchingEachOther(t *testing.T) {
	for _, backend := range openableBackends(t) {
		t.Run(string(backend), func(t *testing.T) {
			...
			// Logging out of one hub must not touch the other.
			removed, err := s.Delete(hubA)
			require.NoError(t, err)
			require.True(t, removed)
...
func TestTheFileFallbackWritesThePathWePredict(t *testing.T) {
	// filePath is a prediction about keyring's internal filename escaping; if
	// it is wrong, checkFileMode stats a file that does not exist, returns
	// nil, and the permission check is silently switched off. So this asks
	// keyring where the item actually landed.
	s, root := fileStore(t)
	require.NoError(t, s.Save(Issued(hubA, sentinelToken, 3600, time.Now())))
...
func TestTheFileFallbackCreatesAnOwnerOnlyFile(t *testing.T) {
	...
	// The resulting mode, not the requested one: keyring creates the file
	// with os.WriteFile(..., 0600), open(2) masks the perm argument with the
	// umask, and a restrictive umask can narrow it further, which is why the
	// assertion is a mask rather than equality with 0600.
...
func TestTheFileFallbackRefusesAFileWiderThanOwnerOnly(t *testing.T) {
	tests := []struct {
		...
		mode    fs.FileMode
		refused bool
		// writable is false for a mode amctl accepts but the OS will not
		// let it write to. 0400 is owner-only, so the permission check has
		// no objection; the EACCES that follows comes from the kernel, not
		// this gate's refusal.
		writable bool
	}{
...
// TestCheckIsTheSameGateBeforeAnythingIsTouched proves Check is the same
// permission arithmetic as Save, callable before a device grant is spent.
// Every row is also asserted against Save, since a Check that disagreed with
// Save would be worse than no pre-flight: login would pass and then fail
// anyway.
func TestCheckIsTheSameGateBeforeAnythingIsTouched(t *testing.T) {
	...
	t.Run("a backend that is not the file one has no permissions to answer for", func(t *testing.T) {
		// On a static darwin build the fallback is `pass`, not a file, and
		// inferring "we must be using a file" from the fact that a fallback
		// happened would make this gate vacuous. Asserted through the file
		// store's own dir turned 0777 with the backend label changed, since
		// there is no `pass` on a test machine.
		s, root := fileStore(t)
		require.NoError(t, os.Chmod(filepath.Join(root, DirName), 0o777))
...
func TestTheFallbackWarningNamesTheBackendActuallyChosen(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// A CGO_ENABLED=0 darwin build lands on `pass`, not on a file,
			// since pass.go precedes FileBackend in keyring's own order.
...
func TestOpenReportsTheFallbackOnTheDiagnosticStream(t *testing.T) {
	...
	// the file backend always opens, and it is never the store a platform's
	// required entry expects, so the warning must fire exactly once.
...
func TestOpenWithADarwinPlatformThatLostItsKeychain(t *testing.T) {
	// End-to-end, on whatever machine the suite runs on: the goos and
	// available hooks are the only way this message is ever exercised, since
	// no macOS runner was available when it was written.
	root := t.TempDir()
	var warnings []string
	s, err := Open(Options{
		StateRoot: root,
...
func TestSaveRefusesACredentialThatCameFromTheEnvironment(t *testing.T) {
	// The guard is the unexported fromEnv field, so a caller cannot resolve
	// a credential and then store it by accident.
	s, root := fileStore(t)
...
func TestIsMissingCoversBothWaysABackendSaysNothingIsThere(t *testing.T) {
	// Most backends return ErrKeyNotFound, but the file backend's Remove
	// passes os.Remove's ENOENT straight through.
