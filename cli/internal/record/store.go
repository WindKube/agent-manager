...
const (
	...
	// tempPrefix starts with a dot and can never be a prefix of FileName, so
	// a leftover temp is never mistakable for the record.
	tempPrefix = "." + FileName + ".amctl-tmp-"
)

// ErrCorrupt marks a record file that could not be read as a record.
// Deliberately not the same outcome as an absent file: an absent record
// means "prune nothing, install everything", the right reading of a machine
// that never synced, while a corrupt one read that way would make a whole
// installed tree permanently unremovable.
...
// does not understand. Never migrated in place: a newer record read by an
// older CLI is a downgrade, and rewriting it at the older version would drop
// whatever the newer one knew.
var ErrSchemaVersion = errors.New("installation record schema version is not supported")

// ErrHubMismatch marks a record that belongs to a different hub — normally a
// directory moved between machines or a hub URL that changed. Refused
// because the record is the authority for what may be removed, so applying
// hub A's record to hub B's tree is a deletion with no evidence behind it.
...
// absent file is an empty record, not an error, since a machine that never
// synced this hub is a normal state. Every other failure is an error naming
...
func Load(path, hub string) (*Record, error) {
	...
	// Read the schema version on its own first, so a future format gets a
	// clear version mismatch instead of a decode failure that reads like
	// corruption. A pointer distinguishes absent from zero.
	var probe struct {
	...
	// DisallowUnknownFields: amctl wrote every byte of this file, so an
	// unknown field at a version we claim to understand means the file was
	// edited or corrupted, and silently dropping it would destroy that
	// information on the next Save.
	...
	// Trailing content is corruption, not slack: exactly what a partially
...
// It writes nothing when the encoded bytes are already on disk, so an
// idempotent sync makes no filesystem modification; this is also why the
// record is normalised into a canonical order. Save must be called after the
// swap, never before (see the package comment). The write is temp-file,
// fsync, rename, fsync-directory: the temp name means an interrupted write
// is invisible to Load, and the fsync before rename prevents a crash from
// leaving the final name present but zero length on a delayed-allocation
// filesystem.
func Save(path string, r *Record) (bool, error) {
	...
	// Indented and newline-terminated: humans debug a prune by reading this file.
...
// syncDir makes the new directory entry durable. Non-fatal: the record is
...
// name prefix alone: a sync holds the per-home lock for its whole duration,
// so at the moment Save completes any temp file present belongs to a
// process that is gone. A prefix match is safe here (unlike in prune)
// because amctl creates and owns this whole directory.
