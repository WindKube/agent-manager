...
where pub.slug = ? and pkg.name = ? and v.semver = ? and v.visible`
...
func Bundle(ctx context.Context, db bun.IDB, publisher, name, version string) (BundleRef, error) {
	var ref BundleRef
	err := db.QueryRowContext(ctx, bundleRefSQL, publisher, name, version).
		Scan(&ref.ObjectKey, &ref.Digest, &ref.Verdict, &ref.SizeBytes)
	switch {
	case errors.Is(err, sql.ErrNoRows):
		return BundleRef{}, ErrNotFound
	case err != nil:
		return BundleRef{}, fmt.Errorf("locate bundle %s/%s@%s: %w", publisher, name, version, err)
	}
	return ref, nil
}
