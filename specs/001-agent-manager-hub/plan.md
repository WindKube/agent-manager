...
`worker fetcher`, `worker scanner` — over PostgreSQL, MinIO and Dex, all started by a
...
| VI. It runs with one command | PASS | `compose.yaml` brings up postgres, minio (+ bucket init), dex, two chained migration init containers, api, web, fetcher, scanner, then `seed` as a one-shot. The Atlas image is third-party infrastructure and does not engage principle I. |
...
| VI | PASS | `quickstart.md` is one command; two chained migration init containers; Dex gives a working login offline. |
...
6. **Dex device-flow parity.** Verify Dex implements the device authorisation grant and
   emits a groups claim, so the local stack exercises the same code path as Okta.
