...
| VII. Workers are declarative plugins | PASS | `worker.Definition` + a single `internal/worker/roles` list; `Needs` drives what the bootstrap constructs; the scanner receives `blob.Reader` with no writer in scope. Same registry pattern for scanner checks and fetch sources. |
...
│   ├── worker/                  # Definition, Needs, Deps — the plugin seam
│   │   ├── roles/               # the one registry; a package, not a file (import cycle)
...
> One deviation from the Technology Constraints table, taken on the evidence R10 demanded, and
> one from `contracts/worker.md` that the compiler forced.
...
| The worker registry is its own package, `internal/worker/roles`, and not `internal/worker/registry.go` as `contracts/worker.md` specified. | A role package imports `internal/worker` for `Definition`, `Needs` and `Deps`. A list in `internal/worker` that names `fetcher.Definition()` is therefore an import cycle and does not compile. The registry has to sit above both the framework and every role, and one level down is the only such place. | `init()`-based self-registration compiles and keeps the list nominally in `internal/worker`, but it trades a compile-time list for an import side effect: the set of roles becomes a function of which packages happen to be linked in, and "the one list a new role touches" (principle VII) stops being something a reader can point at. The contract has been amended instead. |
