...
- [ ] T010 **[R2 GATE]** [P] For each of `claude-code`, `agents-md`, `codex`: find that agent's own documentation for where it loads skills and plugins from, write the layout into `internal/layout/<target>.go` as a doc comment **citing the source** (FR-021), and note whether it was verified by running the agent or only read. A target that cannot be verified is marked unshipped and its constructor returns an error naming R2. **Do not guess a path.**
- [ ] T011 **[R3 GATE]** [P] `internal/apply/swap_test.go` — the rename-aside-rename-in sequence on linux, darwin and windows, with an induced failure at each step, asserting the result is always wholly the old or wholly the new version (FR-024).
...
- [ ] T065 [P] `goreleaser` (or equivalent) for the five platforms, with **`CGO_ENABLED=1` on a macOS runner** per R1 and static elsewhere. Checksums published; the release workflow pinned by SHA like every other action in this repo.
