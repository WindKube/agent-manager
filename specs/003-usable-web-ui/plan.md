...
Two violation-shaped decisions, recorded rather than waved through.
...
| **`internal/auth/oidc.go` grew by about fifty lines where T023's text promised a net simplification.** The task said to remove the `oidc.InsecureIssuerURLContext` path, **keep** `VerifierConfig.DiscoveryURL` and its branch, and come out smaller. Removing the context value while keeping the field made that branch bigger instead: it now fetches the metadata document itself, decodes it under a bounded reader, and compares the document's `issuer` to the configured one. | go-oidc offers exactly two ways to reach a provider whose metadata is served from a host other than its issuer: `InsecureIssuerURLContext`, which turns the issuer comparison OFF, and `ProviderConfig`, which skips discovery and takes the endpoints as given. The check FR-106 wants is neither of them — not "the document came from its own issuer", which is false by construction on this path, and not "no check at all", but "the document names the issuer the operator configured". The library offers no half of that, so it is our code. What shipped is also **stronger** than what it replaced: under the insecure context a metadata host advertising a *third* issuer was accepted and its keys adopted, and that is now refused, with a test. The deviation is in the promised size, not in the intent — T023's own text says to keep the field and the branch. | **Deleting `DiscoveryURL` and its branch outright** — the local stack no longer uses either, so this genuinely would have been a net simplification, and it would have dropped FR-106's support for a real provider whose issuer and discovery host differ. That is a production shape, not a local workaround. **Keeping `InsecureIssuerURLContext`** — fewer lines, and it keeps the hole the fetch-and-compare closed. **Asking go-oidc for the comparison** — there is no option for it; this was checked against the library rather than assumed. |
